...
export async function GET(req: NextRequest) {
  try {
    ...
    return NextResponse.json({ error: "Error loading sessions", details: String(error) }, { status: 500 });
  }
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    return NextResponse.json({ error: "Error creating session", details: String(error) }, { status: 500 });
  }
}

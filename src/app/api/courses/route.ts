...
export async function GET(req: NextRequest) {
  try {
    ...
    return NextResponse.json({ error: "Error loading courses", details: String(error) }, { status: 500 });
  }
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    return NextResponse.json({ error: "Error creating course", details: String(error) }, { status: 500 });
  }
}

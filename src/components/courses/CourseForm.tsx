...
export default function CourseForm({ course, onSave, onCancel }: Props) {
  const [form, setForm] = useState({
    name: course?.name || "",
    entityName: course?.entity?.name || "",
    modality: course?.modality || "presencial",
    cityOrPlatform: course?.cityOrPlatform || "",
    startDate: course?.startDate ? new Date(course.startDate).toISOString().split("T")[0] : "",
    endDate: course?.endDate ? new Date(course.endDate).toISOString().split("T")[0] : "",
    totalHours: course?.totalHours?.toString() || "",
    schedule: course?.schedule || "",
    hourlyRate: course?.hourlyRate?.toString() || "",
    totalPrice: course?.totalPrice?.toString() || "",
    status: course?.status || "pendiente",
    notes: course?.notes || "",
  });
  ...
  const inputClass = "w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent";
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          ...
          <input
            type="text"
            className={inputClass}
            value={form.cityOrPlatform}
            onChange={(e) => setForm({ ...form, cityOrPlatform: e.target.value })}
          />
        </div>
